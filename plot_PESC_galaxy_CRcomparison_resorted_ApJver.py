...
plt.xlabel(r'$\tau_s/T_{Dyn}^{M-}$',fontsize=18)
...
#savefilename='SC_and_PE_CRscan_peakcomplexitysignature_et_dyntimenorm_ApJver_newcolor.png'
...
plt.ylabel(r'$\tau_s^{max}/T_{dyn}$',fontsize=20)
plt.ylim(0,0.1)
plt.text(0.07,0.92,'(a)',horizontalalignment='center',verticalalignment='center',transform=ax1.transAxes,fontsize=16)
...
plt.plot(CRs,np.repeat(20.7,5),color='red',label=r'Mean=20.7 $\pm \sigma=$2.5')
#mean = 20.7 std=2.5
ax3.fill_between(CRs, 20.7-2.5,20.7+2.5, facecolor='lightpink', transform=ax3.transData)
plt.yticks(fontsize=15)
plt.xticks(np.array([4,5,6,7,8]),[4,5,6,7,8],fontsize=18)
ax3.tick_params(axis='x',direction='inout',top=True)
plt.xlabel(r'$R_{CR}$ [kpc]',fontsize=20)
plt.ylabel(r'$T_{D}/\tau_s^{max}$',fontsize=20)
plt.ylim(10,29)
