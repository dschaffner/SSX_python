...
fig=plt.figure(num=1,figsize=(8,6),dpi=600,facecolor='w',edgecolor='k')
...
dlog=np.log10(tot_spec7_1p8[20:400])
flog=np.log10(f[20:400])
A1=np.array([flog,np.ones(len(flog))])
w1=np.linalg.lstsq(A1.T,dlog)[0]
slope=np.round(w1[0],3)
func = f**(slope)
scale_data=tot_spec7_1p8[20]
scale_fit=func[20]
ratio=scale_data/scale_fit

plt.loglog(f,tot_spec7_1p8,'o',color='blue',label=r'$\langle|\widetilde{B}|\rangle$ 1.8kV Spec.Index='+str(np.round(slope,1)))
#plt.loglog(f[20:400],tot_spec7[20:400],)
plt.loglog(f[20:400],3*ratio*func[20:400],color='blue',linestyle='dashed')
