...
def load_picoscope(date,shot_number,maxrange=5,time_range=[-2.0,198.0],location='',plot=False):
    
    probe_dia = 0.003175#m
    probe_dia = 0.00079375#m (1/16'' probe)
    probe_area = np.pi*(probe_dia/2)**2
    startintg_index=0
    meancutoff = 400
    #load file
    #location = 'C:\\Users\\dschaffner\\Google Drive\\Pico Data\\061615\\'
    location = 'C:\\Users\\dschaffner\\OneDrive - brynmawr.edu\\BM2X\\Data Storage\\102918\\'
    #filename='picoscope_'+date+'r ('
    #filename = date+'_mag_shot ('
    filename = date+'_bdr_bdt_shot ('
    ...
    time_s = time_ms*1e-6
    timeB_s = time_s[1:]
    #tindex1 = ssxf.tindex_min(time,time_range[0])
    #tindex2 = ssxf.tindex_min(time,time_range[1])
        
    Bxdot = data[1,:]-np.mean(data[1,0:meancutoff])
    print np.mean(data[1,0:meancutoff])
    neginfs = np.isneginf(Bxdot)
    Bxdot[np.where(neginfs)] = -maxrange
    posinfs = np.isinf(Bxdot)
    Bxdot[np.where(posinfs)] = maxrange
        
    Bydot = data[2,:]-np.mean(data[2,0:meancutoff])
    neginfs = np.isneginf(Bydot)
    Bydot[np.where(neginfs)] = -maxrange
    posinfs = np.isinf(Bydot)
    Bydot[np.where(posinfs)] = maxrange
        
    Bzdot = data[3,:]-np.mean(data[3,0:meancutoff])
    neginfs = np.isneginf(Bzdot)
    Bzdot[np.where(neginfs)] = -maxrange
    posinfs = np.isinf(Bzdot)
    Bzdot[np.where(posinfs)] = maxrange
    ...
    return time_ms,time_s,timeB_s,Bxdot,Bydot,Bzdot,Bx,By,Bz,Bxfilt,Byfilt,Bzfilt,Btot